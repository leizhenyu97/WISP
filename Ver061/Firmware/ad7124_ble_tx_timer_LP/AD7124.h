// Low-level AD7124 register and SPI declarations for the firmware data path.
// This header defines the software shadow-register table and the helpers that read and write the device.
...
// Indexes into the software shadow-register table, not raw wire addresses.
enum ad7124_registers {
     ...
     typedef enum _ad7124_reg_access{
        AD7124_RW = 1,   /* Read and Write */
        AD7124_R  = 2,   /* Read only */
        AD7124_W  = 3,   /* Write only */
    } ad7124_reg_access; // Software access permissions for entries in regs[].

    /*! Device register info */
    typedef struct _ad7124_st_reg {
        int32_t addr;   // Raw AD7124 register address used in the communication byte.
        int32_t value;  // Shadowed register value stored in software.
        uint8_t _size;  // Register payload width in bytes.
        uint8_t rw;     // Access permission from ad7124_reg_access.
    } ad7124_st_reg;

    /* AD7124 raw register addresses placed in the first SPI command byte. */
    enum AD7124_reg_map  {
        COMM_REG     = 0x00,
        STATUS_REG   = 0x00,
        ADC_CTRL_REG = 0x01,
    ...
    /* Communication Register bits: these macros assemble the first SPI command byte. */
...
#define AD7124_ERREN_REG_MM_CRC_ERR_EN         (1 << 1)
    ...
    /* Channel Registers 0-15 bits: map one logical channel to a setup and AINP/AINM pair. */
...
#define AD7124_CH_MAP_REG_AINM(x)      (((x) & 0x1F) << 0)
    ...
    /* Configuration Registers 0-7 bits: polarity, buffering, reference source, and PGA. */
...
#define AD7124_CFG_REG_PGA(x)      (((x) & 0x7) << 0)
    ...
    /* Filter Registers 0-7 bits: filter type, post-filter, single-cycle mode, and full-scale rate. */
...
#define AD7124_CRC8_POLYNOMIAL_REPRESENTATION 0x07 /* CRC polynomial x8 + x2 + x + 1. */
#define AD7124_DISABLE_CRC 0                    /* Driver mode: do not append or verify CRC. */
#define AD7124_USE_CRC 1                        /* Driver mode: append and verify CRC bytes. */
#define AD7124_READ_DATA 2                      /* Driver mode token for data-read transfers. */
...
extern ad7124_st_reg *regs; // Pointer to the active AD7124 shadow-register table.
