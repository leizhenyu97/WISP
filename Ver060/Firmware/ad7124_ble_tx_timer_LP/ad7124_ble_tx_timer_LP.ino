// BLE-triggered AD7124 acquisition loop: rotates an external ADG726 mux across
...
const uint8_t ch_list[CHANNELNUM] = {8,10,12,14};   // External mux channel order for one full scan on the ADG726 address bus.
...
volatile bool SerialSwitch = false;                 // Only gates debug prints; it does not affect telemetry transmission.

BLEUart bleuart;                                    // BLE UART service used for telemetry transmission.

NRF52Timer ITimer0(NRF_TIMER_4);                    // Hardware timer retained for the dormant ISR scheduling path.
  ...
  if (ConfigComplete){
    if (TimeUp){
      if (Bluefruit.connected()){
        ...
        SelectChannel(ch_list[ch_idx_new]);
